...
def _load_run(path):
    ...
    needed = ["primaryExitClass", "stepNumber", "deltaThetaDeg", "process"]
    ...
    for i in range(nrows):
        ...
        data[cls]["all_x"].append(float(step_idx))
        data[cls]["all_y"].append(theta)
        if proc == "msc":
            data[cls]["msc_x"].append(float(step_idx))
...
def _build_profile(xs, ys, n_bins, name):
    p = ROOT.TProfile(name, "", n_bins, 0.5, float(n_bins) + 0.5)
    p.SetDirectory(0)
...
def main():
    parser = argparse.ArgumentParser(
        description="Step-index vs scattering-angle diagnostic plots from PrimaryTrajectoryDiagnostics."
    )
    ...
    x_max = max(r["max_step_idx"] for r in runs)
    x_max = max(x_max, 10)

    c = ROOT.TCanvas("c_step_angle_corr", "Step index vs delta theta", 1800, 900)
    c.Divide(2, 1)
    keep = [c]
    csv_rows = []

    # dummy style references for legend
    ...
    for ipad, cls in enumerate(TARGET_CLASSES, start=1):
        ...
        frame.SetTitle(";Step index;Mean #Delta#theta (deg)")
        frame.Draw("AXIS")
        keep.append(frame)
        ...
        for run in runs:
            ...
            p_all = _build_profile(all_x, all_y, x_max, f"p_all_cls{cls}_step{step_tag}")
            p_msc = _build_profile(msc_x, msc_y, x_max, f"p_msc_cls{cls}_step{step_tag}")
            keep.extend([p_all, p_msc])

            color = _color_for_step(run["step_nm"])
            p_all.SetLineColor(color)
            p_all.SetMarkerColor(color)
            ...
            p_msc.SetMarkerColor(color)
            p_msc.SetLineWidth(3)
            p_msc.SetLineStyle(2)
            p_msc.SetMarkerStyle(24)
    ...
    title.AddText(
        f"Step-index vs #Delta#theta correlation: E0={runs[0]['e0_eV']:.0f} eV, EM={runs[0]['model']}, MC events={runs[0]['events']}"
    )
    title.Draw()
    keep.append(title)

    base = os.path.join(args.output_dir, f"step_index_vs_delta_theta_correlation_class24_{args.label}")
    c.SaveAs(base + ".pdf")
    c.SaveAs(base + ".png")
